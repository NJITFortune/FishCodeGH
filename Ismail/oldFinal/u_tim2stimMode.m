function [outamplitude, outmode] = u_tim2stimMode(spiketimes, modem, indat, intim, offset)
% Usage: outamplitude = u_tim2stim(spiketimes, signal, time, time offset)
% outamplitude = u_tim2stim(curfish.spikes(9).times(curfish.spikes.codes == 3), curfish(9).error_vel, curfish(9).time, -0.1)
%
% This prepares the data for use with the suite of analysis tools for this project.  RUN THIS FIRST!
% This code interpolates the stimulus for each spike time. 

% Sample rate (in seconds) of the stimulus (1/Fs) 
    Sf = intim(2) - intim(1); % Should always be 25Hz
...
for j = length(spiketimes):-1:1
    ...
    % Get the mode for that spike. It is always the previous mode.
    outmode(j) = modem(curidx);
