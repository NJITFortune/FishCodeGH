...
% Calculate the DSIs
evDSI = u_DSItimeplot(curfish(f).spikes.times(curfish(f).spikes.codes == n), curfish(f).error_vel, curfish(f).time);
faDSI = u_DSItimeplot(curfish(f).spikes.times(curfish(f).spikes.codes == n), curfish(f).fish_acc, curfish(f).time);
