...
%% Fish 4, neuron 6 • The acceleration peak is too early?
f = 4; n = 6;
spiketimes = curfish(f).spikes.times(curfish(f).spikes.codes == n);
tim = curfish(f).time;    
    sig = curfish(f).error_vel; 
...
%% Average across 6 of these neurons from 4 fish
% They are 9-3, 9-4, 3-1, 3-2, 3-3, 4-6. (Can include 7-1, 8-6 if you want...)
% We aren't using 7-1 or 8-6 because they aren't easily discriminated in the
% scatterplot.
...
mDSIev = [Penn3EV.dsi] - mean(Penn3EV.rdsi); % 9,3
mDSIev = mDSIev + [Penn4EV.dsi] - mean(Penn4EV.rdsi); % 9,4
mDSIev = mDSIev + [f3n1EV.dsi] - mean(f3n1EV.rdsi); % 3,1
mDSIev = mDSIev + [f3n2EV.dsi] - mean(f3n2EV.rdsi); % 3,2
mDSIev = mDSIev + [f3n3EV.dsi] - mean(f3n3EV.rdsi); % 3,3
% mDSIev = mDSIev + [f7n1EV.dsi] - mean(f7n1EV.rdsi);

mDSIev = mDSIev + [f4n6EV.dsi] - mean(f4n6EV.rdsi); % 4,6
...
figure(101); clf; hold on;
    ...
    text(0.15, -0.15, 'N = 6','FontSize', 18);
