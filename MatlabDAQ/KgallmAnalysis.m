...
for j = length(dataChans):-1:1
   ...
   [bw,flo,fhi,power] = obw(data(sampidx,dataChans(j)), Fs, [200 700]);
    
    %tmp = fftmachine(tmpsig, Fs);
    [peakAmp(j), peakIDX] = max(tmp.fftdata);
    peakFreq(j) = bw.fftfreq(peakIDX);
    sumAmp(j) = sum(bw.fftdata(bw.fftfreq > (peakFreq(j) - rango) & bw.fftfreq < (peakFreq(j) + rango)));
