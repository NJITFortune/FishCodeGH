...
ax(1) = subplot(211); title('single fish'); hold on;
    ...
    for k = 1:length(one)
        %normalize splines around zero
        y = one(k).fftyy2;
        plot(one(k).xx2, (y - mean(y))/max(abs((y - mean(y)))) , 'LineWidth', 2);
...
clear ax;
%spline tranistion summary plot 
figure(32); clf; title('expecting 4 light, get 12'); hold on;
...
ax(1) = subplot(211); title('single fish'); hold on;
    ...
    for k = 1:length(one)
        %normalize splines around zero
        y = one(k).fftyy4;
        plot(one(k).xx4, (y - mean(y))/max(abs((y - mean(y)))) , 'LineWidth', 2);
    ...
    for k = 1:length(dos)
        %normalize splines around zero
        y = dos(k).fftyy4;
        plot(dos(k).xx4, (y - mean(y))/max(abs((y - mean(y)))) , 'LineWidth', 2);
        mmean4 = mmean4 + (y - mean(y))/max(abs((y - mean(y))));
    end
    ...
    for k = 1:length(tres)
        %normalize splines around zero
        y = tres(k).fftyy4;
        plot(tres(k).xx4, (y - mean(y))/max(abs((y - mean(y)))), 'LineWidth', 2);
        mmean4 = mmean4 + (y - mean(y))/max(abs((y - mean(y))));
    end
...
ax(2) = subplot(212); title('multiple fish'); hold on; 
    for kk = 2:length(multi)
        ...
        %high amplitude fish
        plot(multi(kk).hixx4, (yhi - mean(yhi))/max(abs((yhi - mean(yhi)))), 'LineWidth', 2);
        multimean4 = multimean4 + (yhi - mean(yhi))/max(abs((yhi - mean(yhi))));
        
        %low amplitude fish
        plot(multi(kk).loxx4, (ylo - mean(ylo))/max(abs((ylo - mean(ylo)))), 'LineWidth', 2);
