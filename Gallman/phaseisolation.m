...
%% kg data index
 ...
 %get the spline estimates for each sample
    ...
    twidx1 = find(one(k).xx >= one(1).lighttimes(3) & one(k).xx <= (one(1).lighttimes(3) + 48));
    twlightidx = find(one(k).lighttimes >= one(1).lighttimes(3) & one(k).lighttimes <= (one(1).lighttimes(3) + 48));
