function out  = KatieAssembler(userfilespec)
...
% Set up filters
    ...
    % This is important because the fish
...
%% CYCLE THROUGH EVERY FILE IN DIRECTORY
    ...
    for k = length(iFiles):-1:1
       ...
       % Filter data  
            ...
            filtsigCh1 = filtfilt(b,a, data(:,1)); % High pass filter
            filtsigCh1 = filtfilt(f,e,filtsigCh1); % Low pass filter   
            
            filtsigCh2 = filtfilt(b,a, data(:,2)); % High pass filter
            filtsigCh2 = filtfilt(f,e,filtsigCh2); % Low pass filter   

        % PICK YOUR WINDOW - THIS IS A CRITICAL STEP THAT MAY NEED REVISION

            [~, idx] = max(abs(filtsig)); % FIND THE MAXIMUM
