function clrs = k_colormefriendly
%creates color blind friendly color palette but not really
%https://personal.sron.nl/~pault/

%pale colors
pblue = [187/255, 204/255, 238/255];
pcyan = [153/255, 221/255, 255/255];
pgreen = [172/255, 211/255, 158/255];
pink = [255/255, 170/255, 187/255];
BlueSky = [106/255, 90/255, 205/255];

darkpurple = [187/255, 86/255, 149/255];
%pblack = [0,0,0];
pblack = [174/255 1200/255 166/255]; %yellow 
porange2 = [244/255, 165/255, 130/255];
pyellow2 = [254/255, 218/255, 139/255];
mint = [90/255, 174/255, 97/255];
ppurple2 = [194/255, 165/255, 207/255];
purple = [0.4940 0.1840 0.5560];

%pgrey = [221/255, 2221/255, 221/255]; %maybe save for background

clrs = [pblue; pcyan; mint; pink; BlueSky; darkpurple;  porange2; pblack; pyellow2; pgreen;ppurple2; purple];
% 
% %Blue sky
% BlueSky = [106/255, 90/255, 205/255];
% 
% %
% PaleVioletRed = [255/255, 105/255, 180/255];
% 
% %Bluish green
% Bluishgreen = [103/255, 189/255, 170/255];
% 
% %Moderate Red
% Crimson = [220/255, 20/255, 60/255];
% 
% %Orange yellow
% Orangeyellow = [224/255, 163/255, 46/255];
% 
% %Yellow Green
% Yellowgreen = [157/255, 188/255, 64/255];
% 
% %Cyan
% Cyan = [8/255, 133/255, 161/255];
% 
% %Dark skin (brown)
% Brown = [115/255, 82/255, 68/255];
% 
% %coral
% Coral = [255/255, 127/255, 80/255];
% 
% %Foliage (dark green)
% Foliage = [87/255, 108/255, 67/255];
% 
% %Magenta
% darkpurple = [187/255, 86/255, 149/255];
% 
% Black = [0/255, 0/255, 0/255];
% 
% 
% clrs = [Coral; Crimson; Foliage; PaleVioletRed; Bluishgreen; Black; Yellowgreen; Orangeyellow; darkpurple; Cyan; Brown];