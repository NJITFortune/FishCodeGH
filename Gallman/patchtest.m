...
figure(28);clf; hold on;
        ...
        ax(2) = subplot(412); title('Frequency-nonphase'); hold on;
         ...
         ax(3) = subplot(413); title('Frequency-phase'); hold on;
            ...
            plot([out(1).s(y1idx).timcont]/3600, [out(1).s(y1idx).pflo]);
            plot([out(1).s(y1idx).timcont]/3600, [out(1).s(y1idx).pfhi]);
            patch([[out(1).s(y1idx).timcont]/3600 fliplr([out(1).s(y1idx).timcont]/3600)], [[out(1).s(y1idx).pflo] fliplr([out(1).s(y1idx).pfhi])],'b');
            plot([out(1).s(y1idx).timcont]/3600, [out(1).s(y1idx).fftFreq], 'k-');
