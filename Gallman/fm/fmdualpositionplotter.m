function fmdualpositionplotter(in,j)
%% define variables
%non-function usage
...
figure(2); clf; title('fish one velocity'); hold on
        ...
        plot(in.s(j).tim(2:end), medfilt1(doneNose, medfiltnum), 'r');
        plot(in.s(j).tim(2:end), medfilt1(doneFin, medfiltnum), 'g');
        plot(in.s(j).tim(2:end), medfilt1(doneTail, medfiltnum),'b');     
        
        
legend('Nose', 'Dorsal Fin', 'Tail');

figure(3); clf;title('fish two velocity'); hold on

      
        plot(in.s(j).tim(2:end), medfilt1(dtwoNose, medfiltnum), 'r');
        plot(in.s(j).tim(2:end), medfilt1(dtwoFin, medfiltnum), 'g');
        plot(in.s(j).tim(2:end), medfilt1(dtwoTail, medfiltnum),'b');     
