function out = fulldualfishmonger
...
for k = 1:length(iFiles)
    ...
    out.s(k).onenose(:,1) = c(:,2);
    out.s(k).onenose(:,2) = c(:,3);
    out.s(k).onenose(:,3) = c(:,4);
    
    out.s(k).onefin(:,1) = c(:,5);
    out.s(k).onefin(:,2) = c(:,6);
    out.s(k).onefin(:,3) = c(:,7);
    
    out.s(k).onetail(:,1) = c(:,8);
    out.s(k).onetail(:,2) = c(:,9);
    out.s(k).onetail(:,3) = c(:,10);

    out.s(k).twonose(:,1) = c(:,11);
    out.s(k).twonose(:,2) = c(:,12);
    out.s(k).twonose(:,3) = c(:,13);
    
    out.s(k).twofin(:,1) = c(:,14);
    out.s(k).twofin(:,2) = c(:,15);
    out.s(k).twofin(:,3) = c(:,16);
    
    out.s(k).twotail(:,1) = c(:,17);
    out.s(k).twotail(:,2) = c(:,18);
    out.s(k).twotail(:,3) = c(:,19);    
