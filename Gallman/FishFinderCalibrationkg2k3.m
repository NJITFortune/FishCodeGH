...
%     hifishchunk1idx = find(hitube1timff < 76);
        ...
        for j = 1:length(hitube2ampff)
            hitube2ampchunk1(j) = hitube2ampff((j))*2.5;
...
HiAmp = [hitube2ampchunk1, hitube1ampff];
HiTim = [hitube2timchunk1, hitube1timff];
...
%     clear lotube2timchunk1;
    lofishchunk2idx = find(lotube2timff >= 65);
        ...
        for j = 1:length(lofishchunk2idx)
            lotube2ampchunk2(j) = lotube2ampff(lofishchunk2idx(j))*2;
