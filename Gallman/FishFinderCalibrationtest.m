...
%chunk 1 = tim >= 40 & tim < 200
 ...
 hifishchunk1idx = find(hitube2timff < 120);
%     
...
HiAmp = [hitube1ampff,  hitube2ampchunk1, hitube2ampchunk2];
HiTim = [hitube1timff,  hitube2timchunk1, hitube2timchunk2];
HiFreq = [hitube1freqff, hitube2freqff];
...
%% LOW FREQUENCY FISH chunking for calibration
  ...
  %  lofishchunk1idx = find(lotube2timff);
        ...
        for j = 1:length(lotube2timff)
            lotube2ampchunk1(j) = lotube2ampff(j)*2.8;
...
%combine chunks   
LoAmp = [lotube1ampff, lotube2ampchunk1];%, lotube1ampchunk2, lotube1ampchunk3];
LoTim = [lotube1timff, lotube2timchunk1];%, lotube1timchunk2, lotube1timchunk3];
