...
function out = cutKatieLuztrimmer(in)
...
for j = 1:length(lighttimeslong)-1
        ...
        if ~isempty(find([in.e(1).s(tto{1}).uncuttimcont]/(60*60) >= lighttimeslong(j) & [in.e(1).s(tto{1}).uncuttimcont]/(60*60) < (lighttimeslong(j+1)),1))  
            ott = find([in.e(1).s(tto{1}).uncuttimcont]/(60*60) >= lighttimeslong(j) & [in.e(1).s(tto{1}).uncuttimcont]/(60*60) < lighttimeslong(j+1)); 
           lighttim = [in.e(1).s(tto{1}(ott)).uncuttimcont]/(60*60);
           ld = [in.info.ld];
