...
% Get intial frequencies
    ...
    % Put the data into the output structure  
        ...
        out(1).lowdataidx = lowfreqidx(lmaxidx);
        ...
        %high frequency fish
        [~,~,~,e1hipeakobwamp] = obw(e1(hifreqidx(hmaxidx)), Fs, f1.fftfreq(hifreqidx));
        [~,~,~,e2hipeakobwamp] = obw(e2(hifreqidx(hmaxidx)), Fs, f2.fftfreq(hifreqidx));

        out(1).hiamp = max([e1hipeakobwamp, e2hipeakobwamp]);
        out(1).hifreq = currhifreq;
        out(1).hidataidx = hifreqidx(hmaxidx);
...
if fixme == 1
    ...
    % Put the data into the output structure  
        ...
        out(j).hiamp = max([e1hipeakobwamp, e2hipeakobwamp]);
        out(j).hifreq = currhifreq;
        out(j).hidataidx = hifreqidx(hmaxidx);
